...
| 🗂️ **技术组长 · 规划** | `agents/tech-lead-planner.md` | 技术侧重规划、排期的入口变体 |
...
```mermaid
flowchart LR
    IM["💬 飞书 IM<br/>（用户 / 群）"] <-->|HTTP / WS| OPC["🤖 feishu_opc<br/>（FastAPI 等）"]
    OPC -->|HTTPS| EXT["🧠 大模型 API<br/>🛰️ 飞书 OpenAPI"]
    OPC --> PA["📦 project-adapters<br/>（JSON 配置）"]
    OPC --> LA["🔐 .larkagent/secrets<br/>（本地）"]
    OPC --> SK["🎭 skills/<br/>（角色说明）"]
```
